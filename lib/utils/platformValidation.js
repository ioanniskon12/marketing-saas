...
export function validateYouTube(data) {
  ...
  // Get video from media array or video property
  const video = data.video || (data.media && data.media.length > 0 ? data.media[0] : null);
  ...
  // Video must be video type (check both mimeType and mime_type)
  if (video) {
    const mimeType = video.mimeType || video.mime_type || video.type;
