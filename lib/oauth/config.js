...
export const OAUTH_CONFIG = {
  ...
  tiktok: {
    ...
    usesPkce: true, // TikTok requires PKCE
  },
  youtube: {
...
export function getAuthorizationUrl(platform, state) {
  ...
  if (platform === 'youtube') {
    ...
    // Force consent to always get refresh token (even if previously authorized)
  ...
  // TikTok requires PKCE with S256 method
  if (platform === 'tiktok') {
    const codeVerifier = generateCodeVerifier();
    ...
    // Store code_verifier in the state
    const stateData = JSON.parse(Buffer.from(state, 'base64').toString('utf8'));
    stateData.codeVerifier = codeVerifier;
    const newState = Buffer.from(JSON.stringify(stateData)).toString('base64');
    params.set('state', newState);
...
/**
 * Generate S256 code challenge from verifier (for TikTok PKCE)
 */
function generateCodeChallengeS256(verifier) {
  const crypto = require('crypto');
